...
const SimpleElectricalBuilder: React.FC = () => {
  ...
  const buildCircuitRequests = () => {
    ...
    for (const circuit of circuits) {
      // BFS from circuit to collect loads
      const visited = new Set<string>();
      const queue = [circuit.id];
      const loads: any[] = [];
      while (queue.length) {
        const current = queue.shift()!;
        if (visited.has(current)) continue;
        visited.add(current);
        const outgoing = wires.filter(w => w.from === current);
        for (const w of outgoing) {
          const target = components.find(c => c.id === w.to);
          if (!target) continue;
          if (target.type === 'light' || target.type === 'outlet' || target.type === 'appliance' || target.type === 'motor') {
            const amps = target.amps ?? 1;
            const load_type = target.type === 'light' ? 'lighting' : target.type === 'motor' ? 'motor' : target.type === 'appliance' ? 'electronic' : 'resistive';
            loads.push({
              load_id: target.id,
              load_type,
              nominal_voltage: 120,
              nominal_current: amps,
              nominal_power: Math.round(amps * 120),
              power_factor: load_type === 'motor' ? 0.9 : 0.98,
              starting_current_multiplier: load_type === 'motor' ? 5 : 1,
              diversity_factor: 0.8,
              critical_load: false,
              harmonic_content: {},
              operating_schedule: Array(24).fill(0.5),
            })
          }
          queue.push(w.to)
        }
      ...
      const protection_current = circuit.maxAmps ?? 20;
      const req = {
        circuit_id: circuit.id,
        circuit_type: 'branch',
        wire_awg: '12',
        wire_length_ft: 50,
        conduit_fill_percentage: 40,
        ...
        protection_device: {
          ...
          current_rating: protection_current,
          voltage_rating: 120,
          interrupting_rating: 10000,
          trip_curve_type: 'C',
          instantaneous_trip_multiplier: 10,
          time_delay_characteristics: { short_delay_ms: 10 },
        },
      };
      reqs.push(req);
    }
  ...
  const onTestCircuit = async () => {
    setTesting(true);
    setResults([]);
    emitTelemetry({ event_type: 'builder_test_click', payload: { components: components.length, wires: wires.length } });
    try {
      const reqs = buildCircuitRequests();
          ...
          <div className="flex gap-2">
            ...
            <div className="form-control">
              <label className="label cursor-pointer gap-2">
                <span className="label-text text-xs">{proMode ? 'Pro' : 'Kid'} Mode</span>
                <input type="checkbox" className="toggle toggle-primary toggle-xs" checked={proMode} onChange={() => setProMode(v => !v)} />
              </label>
            </div>
          </div>
        </div>
        ...
        <div className="w-80 bg-white border-l-4 border-purple-300 p-4 shadow-lg overflow-y-auto">
          ...
          {results.length > 0 && (
            <div className="mb-4 p-3 bg-blue-50 border-2 border-blue-300 rounded-lg">
              <h3 className="font-bold text-blue-700 mb-2">Deterministic Validation</h3>
              {results.map((r, i) => (
                <div key={i} className="text-xs mb-2">
                  <div className="font-semibold">Circuit {r.circuit_id}: {r.overall_status}</div>
                  {r.constraint_checks?.slice(0,3).map((c: any, idx: number) => (
                    <div key={idx} className={`mt-1 ${c.severity_level === 'critical' ? 'text-red-700' : c.severity_level === 'warning' ? 'text-yellow-700' : 'text-green-700'}`}>
                      • {c.constraint_name}: {c.passes_check ? 'PASS' : 'FAIL'} (margin {Math.round(c.margin_percent)}%)
                    </div>
                  ))}
                </div>
              ))}
